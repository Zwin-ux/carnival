...
mod benchmarks {
	...
	fn attest() {
		...
		let attestation_type = AttestationType::General;
		let score = 50;
		let comment = vec![0u8; 256];

		#[extrinsic_call]
		attest(RawOrigin::Signed(attester), subject, attestation_type, score, comment);
	}
	...
	fn revoke() {
		...
		let score = 50;
		let comment = vec![0u8; 256];
		assert_ok!(Pallet::<T>::attest(RawOrigin::Signed(attester.clone()).into(), subject.clone(), attestation_type, score, comment));
